...
const getNextApplicationPublicId = async () => {
  const lastApplication = await Application.findOne({})
    .sort({ createdAt: -1, _id: -1 })
    .select("publicId");

  if (!lastApplication || !lastApplication.publicId) {
    return "1";
  }

  const numeric = Number.parseInt(lastApplication.publicId, 10);

  if (Number.isNaN(numeric)) {
    const maxNumericApp = await Application.find({
      publicId: { $regex: "^[0-9]+$" },
    })
      .sort({ publicId: -1 })
      .select("publicId");

    if (!maxNumericApp.length) {
      return "1";
    }

    const maxValue = Math.max(
      ...maxNumericApp.map((item) => Number.parseInt(item.publicId, 10)).filter((n) => !Number.isNaN(n))
    );
...
const submitApplication = async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(normalizedJobId)) {
      jobQuery.$or = [{ _id: normalizedJobId }, { publicId: normalizedJobId }];
    } else {
    ...
    const nextPublicId = await getNextApplicationPublicId();
