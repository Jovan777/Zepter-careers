...
} from "../api/adminTalentPoolApi";
import { useAdminAuth } from "../context/AdminAuthContext";
import type {
  AdminTalentPoolAreaOption,
  AdminTalentPoolDetailsResponse,
  AdminTalentPoolListItem,
} from "../types/admin";
...
const formatEventTitle = (type: string) => {
  const map: Record<string, string> = {
    created: "Talent Pool application created",
...
const formatEventValue = (key: string, value: unknown) => {
  ...
  if (typeof value === "string") {
    if ((key === "from" || key === "to" || key.toLowerCase().includes("status")) && statusLabels[value]) {
...
const AdminTalentPoolPage = () => {
  ...
  const [items, setItems] = useState<AdminTalentPoolListItem[]>([]);
  const [statuses, setStatuses] = useState<string[]>([]);
  const [areaOptions, setAreaOptions] = useState<AdminTalentPoolAreaOption[]>([]);
  ...
  const [details, setDetails] = useState<AdminTalentPoolDetailsResponse | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await getAdminTalentPoolApplications(token, {
        search,
        status,
        areaOfInterest,
      });
      ...
      setAreaOptions(data.areaOfInterestOptions);
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : "Greška pri dohvatanju Talent Pool prijava."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const openDetails = async (publicId: string) => {
    ...
    try {
      ...
      const data = await getAdminTalentPoolApplicationById(token, publicId);
      setDetails(data);
      ...
      setDetailsError(
        err instanceof Error
          ? err.message
          : "Greška pri dohvatanju Talent Pool detalja."
      );
    } finally {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      await updateAdminTalentPoolApplicationStatus(
        token,
      ...
      const refreshed = await getAdminTalentPoolApplicationById(
        token,
        details.application.publicId
      );
      ...
      setDetailsError(
        err instanceof Error
          ? err.message
          : "Greška pri izmeni statusa Talent Pool prijave."
      );
    } finally {
      setIsUpdating(false);
    }
  ...
  return (
    <section className="admin-page">
      <div className="admin-page__header">
        <h2>Talent Pool</h2>
        <p>Pregled otvorenih prijava kandidata koji nisu vezani za konkretnu poziciju.</p>
      </div>
      ...
      <div className="admin-panel admin-table-wrapper">
        {isLoading ? (
          <p>Učitavanje Talent Pool prijava...</p>
        ) : (
          <table className="admin-table">
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item._id}>
                  <td>{item.publicId}</td>
                  <td>{new Date(item.submittedAt).toLocaleString("sr-RS")}</td>
                  <td>
                    {item.firstName} {item.lastName}
                  </td>
                  <td>{item.email}</td>
                  <td>{item.phone || "-"}</td>
                  <td>{item.areaOfInterestLabel}</td>
                  <td>
              ...
              {!items.length && (
                <tr>
                  <td colSpan={9}>Nema pronađenih Talent Pool prijava.</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            {application ? (
              <div className="admin-talent-pool-details">
                <div className="admin-details-stack">
                  <p>
                    <strong>Candidate:</strong> {application.firstName}{" "}
                  ...
                  <p>
                    <strong>Area:</strong> {application.areaOfInterestLabel}
                  </p>
                  <p>
                    <strong>Submitted at:</strong>{" "}
                    {new Date(application.submittedAt).toLocaleString("sr-RS")}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Locale:</strong> {application.sourceLocale || "-"}
                  </p>
                  <p>
                    <strong>CV:</strong>{" "}
